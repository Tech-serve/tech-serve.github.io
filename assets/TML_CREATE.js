$(document).ready(function () {
    ...
    function renderSlider(arr){
      arr.map(el => {
        $('.js-slider-for').append(`<img src="./assets/${el}" alt="" />`)
        if(arr.length !== 1) $('.js-slider-nav').append(`<img src="./assets/${el}" alt="" />`)
    ...
    function renderChooseProduct(obj){
      if(obj.length == 1){
        ...
        $('.product-image').prop('src', `./assets/${obj[0].url}`)
        $('.product.chosen-item1').prop('src', `./assets/${obj[0].url}`)
      }
      else{
        obj.map((el, index) => {
          if(index == 0){
            $('.product-image').prop('src', `./assets/${el.url}`)
            $('.product.chosen-item1').prop('src', `./assets/${el.url}`)
